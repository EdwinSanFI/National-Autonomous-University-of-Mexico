...
void insertNodeSpecificPosition(List* list, int value, int position) {
	// Si head no tiene ningun nodo, encontrandose vacío
	int option, valor=1, maximo=1, si;
	if(list->head==NULL){
		// Significa que no hay nodos en la lista
		printf("There are no nodes in the list\n");
		printf("Do you want to insert the node at the beginning?\n");
		printf("1. Yes\n");
		printf("2. No\n");
		scanf("%d", &option);
		if(option == 1) {
			insertNodeBeginning(list, value);
		}
		else {
			printf("Okay :(");
		}
...
int searchNode(List lista, int elemento){
...
void deleteSpecificNode(List* lista,int posicion) {
...
void deleteNodesLessThan(List *lista,int elemento){
    Node *temp = lista->head;
